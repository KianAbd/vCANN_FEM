...
program test_driver
  ...
  ! --- Define toy weights
  real(dp), dimension(3,2) :: W1
  real(dp), dimension(2,3) :: W2
  ! --- Define toy biases
  real(dp), dimension(3)   :: b1
  real(dp), dimension(2)   :: b2

  W1 = reshape([0.5_dp, -0.3_dp, 0.1_dp, 0.7_dp, -0.4_dp, 0.2_dp],[3,2])
  W2 = reshape([0.6_dp, -0.1_dp, 0.2_dp, 0.3_dp, 0.5_dp, -0.2_dp],[2,3])
  ...
  max_in = maxval(nin)
  max_out = maxval(nout)
  allocate(weights(max_out,max_in,L)); weights = 0.0_dp
  weights(1:3,1:2,1) = W1
  weights(1:2,1:3,2) = W2

  allocate(biases(max_out,L)); biases = 0.0_dp
  biases(1:3,1) = b1
  biases(1:2,2) = b2
